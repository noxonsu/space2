...
class YoutubeTelegramScraperProduct(BaseProduct):
    def __init__(self, llm_service: LLMService):
        self.data_loader = ProductDataLoader()
        self.product_data = self.data_loader.load_product_data("youtube_telegram_scraper")
    ...
    def execute_demo(self, demo_params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            query = demo_params.get("query", "Найти Telegram каналы админов") if demo_params else "Найти Telegram каналы админов"
            
            prompt = f"Сгенерируйте демо-результат для YouTube-Telegram скрапера на основе запроса: '{query}'. Покажите найденные каналы."
            llm_response = self.llm_service.generate_text(prompt)
            
            return {
                "status": "success",
                "result": f"Демо-результат YouTube-Telegram скрапера: {llm_response}",
                "demo": True
            }
    ...
    def get_seo_keywords(self) -> list:
        ...
        return self.product_data.get("seo_keywords", [
            "YouTube API автоматизация",
            "Telegram скрапинг",
            "поиск администраторов каналов",
            "холодные продажи автоматизация",
            "YouTube-Telegram интеграция"
        ])
