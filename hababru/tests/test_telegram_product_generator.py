...
"""
    ...
    def test_semantic_duplicate_detection(self):
        """Тест обнаружения семантических дублей с реальными продуктами"""
        # Сначала создаем реальный продукт contract_analysis в тестовой директории
        # (имитируем существующий продукт из системы)
        real_contract_yaml = """---
...
seo:
  keywords:
    ...
    - "юридический анализ"
"""
        # Создаем файл существующего продукта
        existing_product_path = os.path.join(self.temp_dir, "contract_analysis.yaml")
        with open(existing_product_path, 'w', encoding='utf-8') as f:
            f.write(real_contract_yaml)
        
        # Теперь пытаемся создать похожий продукт из Telegram сообщения
        duplicate_message = TelegramMessage(
            message_id=201,
            text="🔍 Новый сервис для проверки договоров с помощью искусственного интеллекта! Анализируем юридические документы и находим риски.",
            date="2025-07-03T16:00:00Z"
        )
        
        # Мокируем LLM для второго вызова (хотя он не должен быть вызван из-за семантической проверки)
        second_yaml = """---
product_id: "new_contract_checker"
name: "Новый анализатор договоров"
description: "Проверка юридических документов с ИИ"
version: "1.0"
category: "legal"
status: "active"
demo_data:
  key_features: ["ИИ анализ", "Проверка рисков"]
product_info:
  key_benefits: ["Быстрая проверка", "Автоматизация"]
seo:
  keywords: ["договор", "проверка", "ИИ", "анализ"]
"""
        ...
        result = self.generator.generate_product_from_message(duplicate_message)
        
        # Проверяем что дубль был обнаружен
        assert result["success"] is False, f"Ожидался неуспех, но результат: {result}"
        assert "duplicate_product_id" in result, f"Ключ 'duplicate_product_id' отсутствует в результате: {result}"
        assert result["duplicate_product_id"] == "contract_analysis"
        assert "похожим смыслом" in result["error"]
...
"""
    ...
    def test_semantic_duplicate_detection_news_analysis(self):
        """Тест обнаружения семантических дублей с продуктом news_analysis"""
        # Создаем реальный продукт news_analysis в тестовой директории
        real_news_yaml = """---
...
demo_data:
  ...
  processing_time: "в реальном времени"
  accuracy: "92%"
...
"""
        # Создаем файл существующего продукта
        existing_product_path = os.path.join(self.temp_dir, "news_analysis.yaml")
        with open(existing_product_path, 'w', encoding='utf-8') as f:
            f.write(real_news_yaml)
        
        # Пытаемся создать похожий продукт из Telegram сообщения
        duplicate_message = TelegramMessage(
            message_id=301,
            text="📰 Умный мониторинг новостей с искусственным интеллектом! Следим за трендами, анализируем упоминания и создаем отчеты в реальном времени.",
            date="2025-07-03T17:00:00Z"
        )
        
        # Мокируем LLM (не должен быть вызван из-за семантической проверки)
        duplicate_yaml = """---
product_id: "smart_news_monitor"
name: "Умный новостной мониторинг"
description: "ИИ-мониторинг новостей и трендов"
version: "1.0"
category: "analytics"
status: "active"
demo_data:
  key_features: ["Мониторинг", "Анализ трендов"]
product_info:
  key_benefits: ["Автоматизация", "Быстрые инсайты"]
seo:
  keywords: ["новости", "мониторинг", "ИИ", "тренды"]
"""
        self.mock_llm_service.generate_text.return_value = duplicate_yaml
        ...
        assert result["success"] is False, f"Ожидался неуспех, но результат: {result}"
        assert "duplicate_product_id" in result, f"Ключ 'duplicate_product_id' отсутствует в результате: {result}"
        assert result["duplicate_product_id"] == "news_analysis"
        assert "похожим смыслом" in result["error"]
