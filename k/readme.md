...
- [ ] **Реализовать класс `Server` (в `src/server.ts`)**
- [ ] **Реализовать класс `Entity` (в `src/entity.ts`)**
- [ ] **Интегрировать `LevelDB` в `Server` для сохранения блоков**
- [ ] Реализовать RLP-сериализацию для блоков
- [ ] **Написать Unit-тесты для `Entity` (в `tests/unit/entity.test.ts`)**
- [ ] **Написать интеграционные тесты для `Server` и `LevelDB` (в `tests/integration/server.test.ts`)**

### Фаза 2: Реализация Account и бизнес-логики
- [ ] Реализовать класс `Account` (в `src/account.ts`)
- [ ] Написать Unit-тесты для `Account` (в `tests/unit/account.test.ts`)
